// Single-op model using QNN built-in ElementWiseExp op (no custom package)
// Used to collect baseline latency for comparing against HEExp custom op.
...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(getQnnGraphConfigFromInfo("QnnExp_graph",
      graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(model.initialize(backendHandle, interface, contextHandle,
      "QnnExp_graph", debug, DO_GRAPH_NODE_VALIDATIONS,
      graphConfigs), err);

  uint32_t dims_in[] = {1, 1, 128, 128};
  VALIDATE(model.addTensor("in",
    (Qnn_Tensor_t){.version = QNN_TENSOR_VERSION_1,
      .v1 = {.id = 0, .name = "in",
        ...
        .rank = 4, .dimensions = dims_in,
        .memType = QNN_TENSORMEMTYPE_RAW,
        .clientBuf = {.data = nullptr, .dataSize = 0}}}), err);

  uint32_t dims_output[] = {1, 1, 128, 128};
  ...
  // QNN built-in ElementWiseExp: package "qti.aisw", op type "ElementWiseExp"
  VALIDATE(model.addNode(QNN_OPCONFIG_VERSION_1,
      "QnnExp_node",                  // node name
      "qti.aisw",                     // package name (built-in)
      QNN_OP_ELEMENT_WISE_EXP,        // "ElementWiseExp"
      nullptr,                        // params
      0,                              // numParams
  ...
  // Hand model out without finalize() — qnn-net-run host driver calls
  // QnnGraph_finalize() itself. Calling finalize twice → segfault (L1).
