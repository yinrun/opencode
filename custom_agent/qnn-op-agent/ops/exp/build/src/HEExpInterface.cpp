...
Qnn_ErrorHandle_t HEExpInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HEExpGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HEExpValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HEExpCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HEExpFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HEExpTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HEExpLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HEExpLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HEExpLogTerminate() {
...
Qnn_ErrorHandle_t HEExpPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HEExpInit;
    interface->v1_4.terminate        = HEExpTerminate;
    interface->v1_4.getInfo          = HEExpGetInfo;
    interface->v1_4.validateOpConfig = HEExpValidateOpConfig;
    interface->v1_4.createOpImpl     = HEExpCreateOpImpl;
    interface->v1_4.freeOpImpl       = HEExpFreeOpImpl;
    interface->v1_4.logInitialize    = HEExpLogInitialize;
    interface->v1_4.logSetLevel      = HEExpLogSetLevel;
    interface->v1_4.logTerminate     = HEExpLogTerminate;
    return QNN_SUCCESS;
}
