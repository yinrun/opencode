...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(getQnnGraphConfigFromInfo("HEMatMul_graph",
      graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(model.initialize(backendHandle, interface, contextHandle,
      "HEMatMul_graph", debug, DO_GRAPH_NODE_VALIDATIONS,
      graphConfigs), err);

  uint32_t dims_a[] = {1, 1, 128, 128};
  ...
  VALIDATE(model.addNode(QNN_OPCONFIG_VERSION_1,
      "HEMatMul_node",  // node name
      "HEMatMulPackage",  // package name
      "HEMatMul",       // op type
      nullptr,           // params
      0,                 // numParams
