...
DECLARE_PKG_OPS_OPTS_LIST(PKG_HEMatMul)
END_PKG_OPS_OPTS_LIST()

static constexpr auto sg_packageName = THIS_PKG_NAME_STR;
static constexpr auto sg_opName      = "HEMatMul";
...
Qnn_ErrorHandle_t HEMatMulInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HEMatMulGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HEMatMulValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HEMatMulCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HEMatMulFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HEMatMulTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HEMatMulLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HEMatMulLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HEMatMulLogTerminate() {
...
Qnn_ErrorHandle_t HEMatMulPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HEMatMulInit;
    interface->v1_4.terminate        = HEMatMulTerminate;
    interface->v1_4.getInfo          = HEMatMulGetInfo;
    interface->v1_4.validateOpConfig = HEMatMulValidateOpConfig;
    interface->v1_4.createOpImpl     = HEMatMulCreateOpImpl;
    interface->v1_4.freeOpImpl       = HEMatMulFreeOpImpl;
    interface->v1_4.logInitialize    = HEMatMulLogInitialize;
    interface->v1_4.logSetLevel      = HEMatMulLogSetLevel;
    interface->v1_4.logTerminate     = HEMatMulLogTerminate;
    return QNN_SUCCESS;
}
