...
Qnn_ErrorHandle_t HECeilInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HECeilGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HECeilValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HECeilCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HECeilFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HECeilTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HECeilLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HECeilLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HECeilLogTerminate() {
...
Qnn_ErrorHandle_t HECeilPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HECeilInit;
    interface->v1_4.terminate        = HECeilTerminate;
    interface->v1_4.getInfo          = HECeilGetInfo;
    interface->v1_4.validateOpConfig = HECeilValidateOpConfig;
    interface->v1_4.createOpImpl     = HECeilCreateOpImpl;
    interface->v1_4.freeOpImpl       = HECeilFreeOpImpl;
    interface->v1_4.logInitialize    = HECeilLogInitialize;
    interface->v1_4.logSetLevel      = HECeilLogSetLevel;
    interface->v1_4.logTerminate     = HECeilLogTerminate;
    return QNN_SUCCESS;
}
