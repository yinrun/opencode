...
static constexpr auto sg_opName      = "HESoftmax";
...
Qnn_ErrorHandle_t SoftmaxInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t SoftmaxGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t SoftmaxValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t SoftmaxCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t SoftmaxFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t SoftmaxTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t SoftmaxLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t SoftmaxLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t SoftmaxLogTerminate() {
...
Qnn_ErrorHandle_t SoftmaxInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = SoftmaxInit;
    interface->v1_4.terminate        = SoftmaxTerminate;
    interface->v1_4.getInfo          = SoftmaxGetInfo;
    interface->v1_4.validateOpConfig = SoftmaxValidateOpConfig;
    interface->v1_4.createOpImpl     = SoftmaxCreateOpImpl;
    interface->v1_4.freeOpImpl       = SoftmaxFreeOpImpl;
    interface->v1_4.logInitialize    = SoftmaxLogInitialize;
    interface->v1_4.logSetLevel      = SoftmaxLogSetLevel;
    interface->v1_4.logTerminate     = SoftmaxLogTerminate;
    return QNN_SUCCESS;
}
