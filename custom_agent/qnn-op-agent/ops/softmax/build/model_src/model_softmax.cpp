// Single-op model for qnn-net-run testing: HESoftmax(input) -> output
// Shape: [1, 14, 128, 128] fp16, reduce over last dim.
...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(getQnnGraphConfigFromInfo("Softmax_graph",
      graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(model.initialize(backendHandle, interface, contextHandle,
      "Softmax_graph", debug, DO_GRAPH_NODE_VALIDATIONS,
      graphConfigs), err);

  uint32_t dims_input[] = {1, 14, 128, 128};
  ...
  uint32_t dims_output[] = {1, 14, 128, 128};
