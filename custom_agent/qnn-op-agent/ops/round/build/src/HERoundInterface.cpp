...
DECLARE_PKG_OPS_OPTS_LIST(PKG_HERound)
END_PKG_OPS_OPTS_LIST()

static constexpr auto sg_packageName = THIS_PKG_NAME_STR;
static constexpr auto sg_opName      = "HERound";
...
Qnn_ErrorHandle_t HERoundInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HERoundGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HERoundValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HERoundCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HERoundFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HERoundTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HERoundLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HERoundLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HERoundLogTerminate() {
...
Qnn_ErrorHandle_t HERoundPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HERoundInit;
    interface->v1_4.terminate        = HERoundTerminate;
    interface->v1_4.getInfo          = HERoundGetInfo;
    interface->v1_4.validateOpConfig = HERoundValidateOpConfig;
    interface->v1_4.createOpImpl     = HERoundCreateOpImpl;
    interface->v1_4.freeOpImpl       = HERoundFreeOpImpl;
    interface->v1_4.logInitialize    = HERoundLogInitialize;
    interface->v1_4.logSetLevel      = HERoundLogSetLevel;
    interface->v1_4.logTerminate     = HERoundLogTerminate;
    return QNN_SUCCESS;
}
