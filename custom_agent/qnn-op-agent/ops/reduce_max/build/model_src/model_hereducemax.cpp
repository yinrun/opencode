...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(getQnnGraphConfigFromInfo("HEReduceMax_graph",
      graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(model.initialize(backendHandle, interface, contextHandle,
      "HEReduceMax_graph", debug, DO_GRAPH_NODE_VALIDATIONS,
      graphConfigs), err);

  uint32_t dims_in0[] = {1, 1, 128, 128};
  ...
  VALIDATE(model.addNode(QNN_OPCONFIG_VERSION_1,
      "HEReduceMax_node",  // node name
      "HEReduceMaxPackage",  // package name
      "HEReduceMax",       // op type
      nullptr,           // params
      0,                 // numParams
