...
Qnn_ErrorHandle_t HEMulInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HEMulGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HEMulValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HEMulCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HEMulFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HEMulTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HEMulLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HEMulLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HEMulLogTerminate() {
...
Qnn_ErrorHandle_t HEMulPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HEMulInit;
    interface->v1_4.terminate        = HEMulTerminate;
    interface->v1_4.getInfo          = HEMulGetInfo;
    interface->v1_4.validateOpConfig = HEMulValidateOpConfig;
    interface->v1_4.createOpImpl     = HEMulCreateOpImpl;
    interface->v1_4.freeOpImpl       = HEMulFreeOpImpl;
    interface->v1_4.logInitialize    = HEMulLogInitialize;
    interface->v1_4.logSetLevel      = HEMulLogSetLevel;
    interface->v1_4.logTerminate     = HEMulLogTerminate;
    return QNN_SUCCESS;
}
