...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(getQnnGraphConfigFromInfo("HENeg_graph", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(model.initialize(backendHandle, interface, contextHandle, "HENeg_graph", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
