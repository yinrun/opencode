...
Qnn_ErrorHandle_t HENegInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HENegGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HENegValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HENegCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HENegFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HENegTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HENegLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HENegLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HENegLogTerminate() {
...
Qnn_ErrorHandle_t HENegPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HENegInit;
    interface->v1_4.terminate        = HENegTerminate;
    interface->v1_4.getInfo          = HENegGetInfo;
    interface->v1_4.validateOpConfig = HENegValidateOpConfig;
    interface->v1_4.createOpImpl     = HENegCreateOpImpl;
    interface->v1_4.freeOpImpl       = HENegFreeOpImpl;
    interface->v1_4.logInitialize    = HENegLogInitialize;
    interface->v1_4.logSetLevel      = HENegLogSetLevel;
    interface->v1_4.logTerminate     = HENegLogTerminate;
    return QNN_SUCCESS;
}
