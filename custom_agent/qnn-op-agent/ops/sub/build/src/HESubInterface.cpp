...
Qnn_ErrorHandle_t HESubInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HESubGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HESubValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HESubCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HESubFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HESubTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HESubLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HESubLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HESubLogTerminate() {
...
Qnn_ErrorHandle_t HESubPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HESubInit;
    interface->v1_4.terminate        = HESubTerminate;
    interface->v1_4.getInfo          = HESubGetInfo;
    interface->v1_4.validateOpConfig = HESubValidateOpConfig;
    interface->v1_4.createOpImpl     = HESubCreateOpImpl;
    interface->v1_4.freeOpImpl       = HESubFreeOpImpl;
    interface->v1_4.logInitialize    = HESubLogInitialize;
    interface->v1_4.logSetLevel      = HESubLogSetLevel;
    interface->v1_4.logTerminate     = HESubLogTerminate;
    return QNN_SUCCESS;
}
