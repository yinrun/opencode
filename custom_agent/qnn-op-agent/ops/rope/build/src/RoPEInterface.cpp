...
static constexpr auto sg_opName      = "HERoPE";
...
Qnn_ErrorHandle_t RoPEInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t RoPEGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t RoPEValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t RoPECreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t RoPEFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t RoPETerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t RoPELogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t RoPELogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t RoPELogTerminate() {
...
Qnn_ErrorHandle_t RoPEInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = RoPEInit;
    interface->v1_4.terminate        = RoPETerminate;
    interface->v1_4.getInfo          = RoPEGetInfo;
    interface->v1_4.validateOpConfig = RoPEValidateOpConfig;
    interface->v1_4.createOpImpl     = RoPECreateOpImpl;
    interface->v1_4.freeOpImpl       = RoPEFreeOpImpl;
    interface->v1_4.logInitialize    = RoPELogInitialize;
    interface->v1_4.logSetLevel      = RoPELogSetLevel;
    interface->v1_4.logTerminate     = RoPELogTerminate;
    return QNN_SUCCESS;
}
