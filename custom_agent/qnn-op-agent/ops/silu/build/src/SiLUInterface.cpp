...
static constexpr auto sg_opName      = "HESiLU";
...
Qnn_ErrorHandle_t SiLUInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t SiLUGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t SiLUValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t SiLUCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t SiLUFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t SiLUTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t SiLULogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t SiLULogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t SiLULogTerminate() {
...
Qnn_ErrorHandle_t SiLUInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = SiLUInit;
    interface->v1_4.terminate        = SiLUTerminate;
    interface->v1_4.getInfo          = SiLUGetInfo;
    interface->v1_4.validateOpConfig = SiLUValidateOpConfig;
    interface->v1_4.createOpImpl     = SiLUCreateOpImpl;
    interface->v1_4.freeOpImpl       = SiLUFreeOpImpl;
    interface->v1_4.logInitialize    = SiLULogInitialize;
    interface->v1_4.logSetLevel      = SiLULogSetLevel;
    interface->v1_4.logTerminate     = SiLULogTerminate;
    return QNN_SUCCESS;
}
