...
DECLARE_PKG_OPS_OPTS_LIST(PKG_HEFloor)
END_PKG_OPS_OPTS_LIST()

static constexpr auto sg_packageName = THIS_PKG_NAME_STR;
static constexpr auto sg_opName      = "HEFloor";
...
Qnn_ErrorHandle_t HEFloorInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HEFloorGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HEFloorValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HEFloorCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HEFloorFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HEFloorTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HEFloorLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HEFloorLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HEFloorLogTerminate() {
...
Qnn_ErrorHandle_t HEFloorPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HEFloorInit;
    interface->v1_4.terminate        = HEFloorTerminate;
    interface->v1_4.getInfo          = HEFloorGetInfo;
    interface->v1_4.validateOpConfig = HEFloorValidateOpConfig;
    interface->v1_4.createOpImpl     = HEFloorCreateOpImpl;
    interface->v1_4.freeOpImpl       = HEFloorFreeOpImpl;
    interface->v1_4.logInitialize    = HEFloorLogInitialize;
    interface->v1_4.logSetLevel      = HEFloorLogSetLevel;
    interface->v1_4.logTerminate     = HEFloorLogTerminate;
    return QNN_SUCCESS;
}
