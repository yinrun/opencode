...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(getQnnGraphConfigFromInfo("HEFloor_graph", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(model.initialize(backendHandle, interface, contextHandle, "HEFloor_graph", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);

  uint32_t dims_in[] = {1, 1, 128, 896};
  ...
  VALIDATE(model.addNode(QNN_OPCONFIG_VERSION_1,
      "HEFloor_node", "HEFloorPackage", "HEFloor",
