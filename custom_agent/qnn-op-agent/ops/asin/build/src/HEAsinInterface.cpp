...
Qnn_ErrorHandle_t HEAsinInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HEAsinGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HEAsinValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HEAsinCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HEAsinFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HEAsinTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HEAsinLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HEAsinLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HEAsinLogTerminate() {
...
Qnn_ErrorHandle_t HEAsinPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HEAsinInit;
    interface->v1_4.terminate        = HEAsinTerminate;
    interface->v1_4.getInfo          = HEAsinGetInfo;
    interface->v1_4.validateOpConfig = HEAsinValidateOpConfig;
    interface->v1_4.createOpImpl     = HEAsinCreateOpImpl;
    interface->v1_4.freeOpImpl       = HEAsinFreeOpImpl;
    interface->v1_4.logInitialize    = HEAsinLogInitialize;
    interface->v1_4.logSetLevel      = HEAsinLogSetLevel;
    interface->v1_4.logTerminate     = HEAsinLogTerminate;
    return QNN_SUCCESS;
}
