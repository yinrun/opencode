...
DECLARE_PKG_OPS_OPTS_LIST(PKG_HESigmoid)
END_PKG_OPS_OPTS_LIST()

static constexpr auto sg_packageName = THIS_PKG_NAME_STR;
static constexpr auto sg_opName      = "HESigmoid";
...
Qnn_ErrorHandle_t HESigmoidInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HESigmoidGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HESigmoidValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HESigmoidCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HESigmoidFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HESigmoidTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HESigmoidLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HESigmoidLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HESigmoidLogTerminate() {
...
Qnn_ErrorHandle_t HESigmoidPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HESigmoidInit;
    interface->v1_4.terminate        = HESigmoidTerminate;
    interface->v1_4.getInfo          = HESigmoidGetInfo;
    interface->v1_4.validateOpConfig = HESigmoidValidateOpConfig;
    interface->v1_4.createOpImpl     = HESigmoidCreateOpImpl;
    interface->v1_4.freeOpImpl       = HESigmoidFreeOpImpl;
    interface->v1_4.logInitialize    = HESigmoidLogInitialize;
    interface->v1_4.logSetLevel      = HESigmoidLogSetLevel;
    interface->v1_4.logTerminate     = HESigmoidLogTerminate;
    return QNN_SUCCESS;
}
