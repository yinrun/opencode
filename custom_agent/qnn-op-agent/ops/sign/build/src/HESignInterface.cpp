...
Qnn_ErrorHandle_t HESignInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HESignGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HESignValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HESignCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HESignFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HESignTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HESignLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HESignLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HESignLogTerminate() {
...
Qnn_ErrorHandle_t HESignPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HESignInit;
    interface->v1_4.terminate        = HESignTerminate;
    interface->v1_4.getInfo          = HESignGetInfo;
    interface->v1_4.validateOpConfig = HESignValidateOpConfig;
    interface->v1_4.createOpImpl     = HESignCreateOpImpl;
    interface->v1_4.freeOpImpl       = HESignFreeOpImpl;
    interface->v1_4.logInitialize    = HESignLogInitialize;
    interface->v1_4.logSetLevel      = HESignLogSetLevel;
    interface->v1_4.logTerminate     = HESignLogTerminate;
    return QNN_SUCCESS;
}
