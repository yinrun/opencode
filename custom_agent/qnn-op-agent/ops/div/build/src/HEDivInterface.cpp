...
Qnn_ErrorHandle_t HEDivInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HEDivGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HEDivValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HEDivCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HEDivFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HEDivTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HEDivLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HEDivLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HEDivLogTerminate() {
...
Qnn_ErrorHandle_t HEDivPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HEDivInit;
    interface->v1_4.terminate        = HEDivTerminate;
    interface->v1_4.getInfo          = HEDivGetInfo;
    interface->v1_4.validateOpConfig = HEDivValidateOpConfig;
    interface->v1_4.createOpImpl     = HEDivCreateOpImpl;
    interface->v1_4.freeOpImpl       = HEDivFreeOpImpl;
    interface->v1_4.logInitialize    = HEDivLogInitialize;
    interface->v1_4.logSetLevel      = HEDivLogSetLevel;
    interface->v1_4.logTerminate     = HEDivLogTerminate;
    return QNN_SUCCESS;
}
