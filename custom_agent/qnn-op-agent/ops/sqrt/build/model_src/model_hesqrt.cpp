// Auto-generated — uses getGraphInfoFromModels (NOT model.finalize) per AGENTS.md L1.
...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(getQnnGraphConfigFromInfo("HESqrt_graph", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(model.initialize(backendHandle, interface, contextHandle, "HESqrt_graph", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  ...
  VALIDATE(model.addNode(QNN_OPCONFIG_VERSION_1,
      "HESqrt_node", "HESqrtPackage", "HESqrt",
      nullptr, 0,
      inputNames, 1,
      outputTensors, 1), err);
