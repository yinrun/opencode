...
Qnn_ErrorHandle_t HESqrtInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HESqrtGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HESqrtValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HESqrtCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HESqrtFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HESqrtTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HESqrtLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HESqrtLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HESqrtLogTerminate() {
...
Qnn_ErrorHandle_t HESqrtPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HESqrtInit;
    interface->v1_4.terminate        = HESqrtTerminate;
    interface->v1_4.getInfo          = HESqrtGetInfo;
    interface->v1_4.validateOpConfig = HESqrtValidateOpConfig;
    interface->v1_4.createOpImpl     = HESqrtCreateOpImpl;
    interface->v1_4.freeOpImpl       = HESqrtFreeOpImpl;
    interface->v1_4.logInitialize    = HESqrtLogInitialize;
    interface->v1_4.logSetLevel      = HESqrtLogSetLevel;
    interface->v1_4.logTerminate     = HESqrtLogTerminate;
    return QNN_SUCCESS;
}
