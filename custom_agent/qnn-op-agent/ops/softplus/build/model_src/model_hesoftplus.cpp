// Manually linked model — uses getGraphInfoFromModels per AGENTS.md L1/L7/L12
...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(getQnnGraphConfigFromInfo("HESoftplus_graph", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(model.initialize(backendHandle, interface, contextHandle, "HESoftplus_graph", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);

  uint32_t dims_in[] = {1, 1, 128, 4864};
  ...
  VALIDATE(model.addNode(QNN_OPCONFIG_VERSION_1,
      "HESoftplus_node", "HESoftplusPackage", "HESoftplus",
      nullptr, 0, inputNames, 1, outputTensors, 1), err);
