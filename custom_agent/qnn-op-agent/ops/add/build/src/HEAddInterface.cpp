...
Qnn_ErrorHandle_t HEAddInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HEAddGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HEAddValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HEAddCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HEAddFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HEAddTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HEAddLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HEAddLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HEAddLogTerminate() {
...
Qnn_ErrorHandle_t HEAddPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HEAddInit;
    interface->v1_4.terminate        = HEAddTerminate;
    interface->v1_4.getInfo          = HEAddGetInfo;
    interface->v1_4.validateOpConfig = HEAddValidateOpConfig;
    interface->v1_4.createOpImpl     = HEAddCreateOpImpl;
    interface->v1_4.freeOpImpl       = HEAddFreeOpImpl;
    interface->v1_4.logInitialize    = HEAddLogInitialize;
    interface->v1_4.logSetLevel      = HEAddLogSetLevel;
    interface->v1_4.logTerminate     = HEAddLogTerminate;
    return QNN_SUCCESS;
}
