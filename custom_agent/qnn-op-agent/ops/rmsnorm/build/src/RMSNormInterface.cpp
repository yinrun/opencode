...
static constexpr auto sg_opName      = "HERmsNorm";
...
Qnn_ErrorHandle_t RMSNormValidateOpConfig(Qnn_OpConfig_t opConfig) {
    ...
    if (std::string(opConfig.v1.typeName) == sg_opName) {
        // RMSNorm: 2 inputs (x, weight), 1 output
...
Qnn_ErrorHandle_t RMSNormCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
