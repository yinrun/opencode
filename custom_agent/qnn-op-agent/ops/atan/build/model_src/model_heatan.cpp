...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(getQnnGraphConfigFromInfo("HEAtan_graph", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(model.initialize(backendHandle, interface, contextHandle, "HEAtan_graph", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  ...
  VALIDATE(model.addNode(QNN_OPCONFIG_VERSION_1,
      "HEAtan_node", "HEAtanPackage", "HEAtan",
