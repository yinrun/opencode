...
Qnn_ErrorHandle_t HEAtanInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HEAtanGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HEAtanValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HEAtanCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HEAtanFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HEAtanTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HEAtanLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HEAtanLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HEAtanLogTerminate() {
...
Qnn_ErrorHandle_t HEAtanPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HEAtanInit;
    interface->v1_4.terminate        = HEAtanTerminate;
    interface->v1_4.getInfo          = HEAtanGetInfo;
    interface->v1_4.validateOpConfig = HEAtanValidateOpConfig;
    interface->v1_4.createOpImpl     = HEAtanCreateOpImpl;
    interface->v1_4.freeOpImpl       = HEAtanFreeOpImpl;
    interface->v1_4.logInitialize    = HEAtanLogInitialize;
    interface->v1_4.logSetLevel      = HEAtanLogSetLevel;
    interface->v1_4.logTerminate     = HEAtanLogTerminate;
    return QNN_SUCCESS;
}
