...
DECLARE_PKG_OPS_OPTS_LIST(PKG_SwiGLU)
END_PKG_OPS_OPTS_LIST()

static constexpr auto sg_packageName = THIS_PKG_NAME_STR;
static constexpr auto sg_opName      = "HESwiGLU";
...
Qnn_ErrorHandle_t SwiGLUInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t SwiGLUGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t SwiGLUValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t SwiGLUCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t SwiGLUFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t SwiGLUTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t SwiGLULogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t SwiGLULogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t SwiGLULogTerminate() {
...
Qnn_ErrorHandle_t SwiGLUInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = SwiGLUInit;
    interface->v1_4.terminate        = SwiGLUTerminate;
    interface->v1_4.getInfo          = SwiGLUGetInfo;
    interface->v1_4.validateOpConfig = SwiGLUValidateOpConfig;
    interface->v1_4.createOpImpl     = SwiGLUCreateOpImpl;
    interface->v1_4.freeOpImpl       = SwiGLUFreeOpImpl;
    interface->v1_4.logInitialize    = SwiGLULogInitialize;
    interface->v1_4.logSetLevel      = SwiGLULogSetLevel;
    interface->v1_4.logTerminate     = SwiGLULogTerminate;
    return QNN_SUCCESS;
}
