...
DECLARE_PKG_OPS_OPTS_LIST(PKG_HERsqrt)
END_PKG_OPS_OPTS_LIST()

static constexpr auto sg_packageName = THIS_PKG_NAME_STR;
static constexpr auto sg_opName      = "HERsqrt";
...
Qnn_ErrorHandle_t HERsqrtInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t HERsqrtGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t HERsqrtValidateOpConfig(Qnn_OpConfig_t opConfig) {
...
Qnn_ErrorHandle_t HERsqrtCreateOpImpl(QnnOpPackage_GraphInfrastructure_t graphInfra,
...
Qnn_ErrorHandle_t HERsqrtFreeOpImpl(QnnOpPackage_OpImpl_t opImpl) {
    (void)opImpl;
    return QNN_OP_PACKAGE_ERROR_UNSUPPORTED_FEATURE;
}

Qnn_ErrorHandle_t HERsqrtTerminate() {
    if (!sg_initialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HERsqrtLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t HERsqrtLogSetLevel(QnnLog_Level_t maxLogLevel) {
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t HERsqrtLogTerminate() {
...
Qnn_ErrorHandle_t HERsqrtPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
    if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
    interface->interfaceVersion      = {1, 4, 0};
    interface->v1_4.init             = HERsqrtInit;
    interface->v1_4.terminate        = HERsqrtTerminate;
    interface->v1_4.getInfo          = HERsqrtGetInfo;
    interface->v1_4.validateOpConfig = HERsqrtValidateOpConfig;
    interface->v1_4.createOpImpl     = HERsqrtCreateOpImpl;
    interface->v1_4.freeOpImpl       = HERsqrtFreeOpImpl;
    interface->v1_4.logInitialize    = HERsqrtLogInitialize;
    interface->v1_4.logSetLevel      = HERsqrtLogSetLevel;
    interface->v1_4.logTerminate     = HERsqrtLogTerminate;
    return QNN_SUCCESS;
}
