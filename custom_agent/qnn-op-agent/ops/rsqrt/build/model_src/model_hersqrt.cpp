...
ModelError_t QnnModel_composeGraphs(
  ...
  VALIDATE(model.addNode(QNN_OPCONFIG_VERSION_1,
      "HERsqrt_node",  // node name
      "HERsqrtPackage",  // package name
      "HERsqrt",       // op type
      nullptr,           // params
      0,                 // numParams
  ...
  // Do NOT call model.finalize() — qnn-net-run host driver calls
  // QnnGraph_finalize() itself. Calling finalize twice causes graph-prepare
  // to segfault / Context Creation failure.
  // (See AGENTS.md L1; same fix as silu/exp/mul.)
